...
try {
    ...
    const res = await axios.post("https://todobackend-ps81.onrender.com/api/auth/register", {
        username : Username,
        email : Email,
        password : Password,
...
<input value={Username} onChange={ (e) => {setUsername(e.target.value)}} type="text" className='w-[80%] h-10 rounded-2xl border-2' placeholder='Username' />
...
<button onClick={HandleRegister} className='bg-green-500 py-2 px-4 rounded-2xl text-white font-bold hover:scale-105 active:scale-95 transition cursor-pointer' >Create Account</button>
